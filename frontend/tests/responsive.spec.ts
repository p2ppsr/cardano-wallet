...
test('wallet shell is responsive and non-overflowing', async ({ page }) => {
  ...
  await page.goto('/')
  await expect(page.getByRole('heading', { name: 'Cardano Wallet' })).toBeVisible()
  await expect(page.getByText('For epochs, vaults, and improbable bridges.')).toBeVisible()
  await expect(page.getByRole('button', { name: /open cardano vault/i })).toBeVisible()
...
test('wide short viewport preserves the Cardano hero composition', async ({ page }, testInfo) => {
